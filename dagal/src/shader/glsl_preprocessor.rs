...
impl IncludeContext {
	...
	pub fn resolve_include(&mut self, source_path: PathBuf, include_path: PathBuf) -> Result<ResolvedInclude> {
		if self.include_stack.contains(&include_path) {
			return Err(anyhow::anyhow!(format!("Invalid #include usage found in {:?}. Trying to include {:?}", &source_path, &include_path)));
		...
		let res = if include_path.exists() {
			...
			Err(anyhow::anyhow!(format!("Tried to #include for {:?} in {:?}. Does not exist.", &include_path, &source_path)))
		};
