...
impl ImmediateSubmit {
	pub fn new(device: crate::device::LogicalDevice, queue: crate::device::Queue) -> Result<Self> {
		...
		let command_pool = crate::command::CommandPool::new(device.clone(), &queue, vk::CommandPoolCreateFlags::empty())?;
	...
	pub fn submit(&self, function:  Box<dyn FnOnce(crate::device::LogicalDevice, &crate::command::CommandBufferRecording)>) {
		unsafe {
			self.device.get_handle().reset_fences(&[self.fence.handle()]).unwrap();
			self.command_buffer.reset(vk::CommandBufferResetFlags::empty()).unwrap();
		}
		let cmd = self.command_buffer.clone().begin(
			vk::CommandBufferUsageFlags::ONE_TIME_SUBMIT
		).unwrap();
		function(self.device.clone(), &cmd);
		let cmd = cmd.end().unwrap();
		let raw_cmd = cmd.handle();
		cmd.submit(self.queue.handle(), &[crate::command::CommandBufferExecutable::submit_info_sync(&[
			crate::command::CommandBufferExecutable::submit_info(raw_cmd)
		], &[], &[])], self.fence.handle()).unwrap();
		unsafe {
