...
impl VirtualResourceStorage {
    ...
    pub fn insert<T: super::VirtualableResource>(&mut self, description: T::Description) -> super::ResourceId {
        let internal_idx: usize = self.items.len();
        let inner_slot_idx = self.free_list.pop().unwrap_or({
            let idx = self.inner.len();
            self.inner.push(InternalSlot { id: 0, generation: 0});
            idx
        });
    ...
    pub(crate) fn get_item<T: super::VirtualableResource>(&self, id: super::ResourceId) -> Option<&InnerBox> {
    ...
    pub fn get_description<T: super::VirtualableResource>(&self, id: super::ResourceId) -> Option<&T::Description> {
        self.get_item::<T>(id).and_then(|i| i.item.downcast_ref::<InnerItem<T>>()).map(|v| &v.description)
    }
    
    pub(crate) fn get_physical<T: super::VirtualableResource>(&self, id: super::ResourceId) -> Option<&Physical<T>> {
        self.get_item::<T>(id).and_then(|i| i.item.downcast_ref::<InnerItem<T>>()).and_then(|v| v.physical.as_ref())
    }
    
    /// Returns [`Some`] if item was successfully inserted
    pub fn insert_physical<T: super::VirtualableResource>(&mut self, id: super::ResourceId, physical: T::Physical, state: T::PhysicalStore) -> Option<()> {
        self.mut_inspect_item(id, |item: &mut InnerItem<T>| {
            item.physical.replace(Physical { physical, state });
        }).map(|_| Some(())).unwrap_or(None)
    }
    
    pub fn get_physical_store<T: super::VirtualableResource>(&self, id: super::ResourceId) -> Option<&T::Physical> {
        self.get_item::<T>(id)
    ...
    pub fn get_physical_state<T: super::VirtualableResource>(&self, id: super::ResourceId) -> Option<&T::PhysicalStore> {
        self.get_item::<T>(id)
            .and_then(|i| i.item.downcast_ref::<InnerItem<T>>())
            .and_then(|i| i.physical.as_ref())
            .map(|p| &p.state)
    ...
    pub(crate) fn mut_inspect_item<T: super::VirtualableResource, O, F: FnOnce(&mut InnerItem<T>) -> O>(&mut self, id: super::ResourceId, f: F) -> Option<O> {
    ...
    pub(crate) fn remove<T: super::VirtualableResource>(&mut self, id: super::ResourceId) -> Option<Box<InnerItem<T>>> {
        ...
        if inner.id as usize != last_idx {
            ...
            self.inner.get_mut(
                self.items.get(inner.id as usize).unwrap().back_index
            ).unwrap().id = inner.id;
            // free up inner slot
            self.inner.get_mut(id.id() as usize).unwrap().generation += 1;
            self.free_list.push(id.id() as usize);
        }
        self.items.pop().and_then(|i| i.item.downcast::<InnerItem<T>>().ok())
    }
}
