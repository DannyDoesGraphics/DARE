...
impl<A: Allocator> ArcAllocator<A> {
	...
	pub fn allocate(&mut self, name: &str, requirements: &vk::MemoryRequirements, ty: super::MemoryLocation) -> Result<ArcAllocation<A>> {
...
impl<A: Allocator> ArcAllocation<A> {
	pub fn offset(&self) -> Result<vk::DeviceSize> {
		self.allocation
			...
			.unwrap_or_else(|| Err(anyhow::Error::from(crate::DagalError::EmptyMemoryAllocation)))
	}
...
impl<A: Allocator> Destructible for ArcAllocation<A> {
	fn destroy(&mut self) {
		self.allocation.write().map(|mut allocation| {
			allocation.take().map(|allocation| {
