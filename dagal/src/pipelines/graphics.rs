...
pub struct GraphicsPipelineBuilder<'a> {
	shaders: HashMap<vk::ShaderStageFlags, vk::PipelineShaderStageCreateInfo<'a>>,
...
impl<'a> super::PipelineBuilder for GraphicsPipelineBuilder<'a> {
	...
	fn replace_shader(mut self, shader: crate::shader::Shader, stage: vk::ShaderStageFlags) -> Self {
		self.shaders.insert(stage, vk::PipelineShaderStageCreateInfo {
	...
	fn build(mut self, device: crate::device::LogicalDevice) -> anyhow::Result<Self::BuildTo> {
		...
		let dynamic_states: Vec<vk::DynamicState> = vec![vk::DynamicState::VIEWPORT, vk::DynamicState::SCISSOR];
		...
		let p_stages =  self.shaders.values().cloned().map(|mut info| {
			info.p_name = entry;
			info
		...
		let pipeline_info = vk::GraphicsPipelineCreateInfo {
			...
			layout: self.layout.unwrap().handle(),
		...
		let handle = unsafe {
			device.get_handle().create_graphics_pipelines(vk::PipelineCache::null(), &[pipeline_info], None).unwrap()
		}.pop().unwrap();
...
impl<'a> GraphicsPipelineBuilder<'a> {
	...
	pub fn set_shader(mut self, shader: crate::shader::Shader, stage: vk::ShaderStageFlags) -> Self {
		self.shaders.insert(stage, vk::PipelineShaderStageCreateInfo {
			s_type: vk::StructureType::PIPELINE_SHADER_STAGE_CREATE_INFO,
			p_next: ptr::null(),
			flags: vk::PipelineShaderStageCreateFlags::empty(),
