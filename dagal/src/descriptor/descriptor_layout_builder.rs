...
impl<'a> DescriptorLayoutBuilder<'a> {
	...
	pub fn build(mut self, device: crate::device::LogicalDevice, shader_stages: vk::ShaderStageFlags, p_next: *const c_void, create_flags: vk::DescriptorSetLayoutCreateFlags) -> Result<crate::descriptor::DescriptorSetLayout> {
		...
		let handle = unsafe {
			device.get_handle().create_descriptor_set_layout(&descriptor_set_layout_ci, None)?
		};
		Ok(crate::descriptor::DescriptorSetLayout::from_raw(handle, device))
	}
}