...
impl<A: Allocator> GrowableBuffer<A> {
    ...
    pub fn new(ci: resource::BufferCreateInfo<A>) -> Result<Self> {
        let (flags, location, device) = match &ci {
            resource::BufferCreateInfo::NewEmptyBuffer { usage_flags, memory_type, device, .. } => {
                (*usage_flags, *memory_type, device.clone())
            }
        };
        if flags & vk::BufferUsageFlags::TRANSFER_DST != vk::BufferUsageFlags::TRANSFER_DST || flags & vk::BufferUsageFlags::TRANSFER_SRC != vk::BufferUsageFlags::TRANSFER_SRC {
    ...
    pub fn update_buffer(&mut self, allocator: &mut ArcAllocator<A>, immediate_submit: &mut dagal::util::ImmediateSubmit, new_size: vk::DeviceSize) -> Result<()> {
        if self.handle.get_size() >= new_size {
            return Ok(())
        }
        // copy the buffers over
        let mut extended_buffer = resource::Buffer::<A>::new(resource::BufferCreateInfo::NewEmptyBuffer {
        ...
        if let Some(debug_utils) = immediate_submit.get_device().clone().get_debug_utils().as_ref() {
            extended_buffer.set_name(debug_utils, "Mesh Info Buffer")?;
        }
        immediate_submit.submit(|ctx| {
            unsafe {
