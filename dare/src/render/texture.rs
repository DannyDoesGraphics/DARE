...
impl<A: Allocator> Texture<A> {
    pub fn new(image: Handle<resource::Image<A>>, sampler: Handle<resource::Sampler>, gpu_rt: GPUResourceTable<A>) -> Self {
        Self {
    ...
    pub fn get_image(&self) -> &handle::ImageHandle<A> {
        &self.image
    }

    pub fn get_sampler(&self) -> &handle::SamplerHandle<A> {
        &self.sampler
    }
}