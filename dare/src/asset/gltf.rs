...
impl<A: Allocator + 'static> GLTFLoader<A> {
    ...
    pub fn load(
        ...
        asset_manager: becs::Res<asset::AssetManager<A>>,
        path: std::path::PathBuf,
    ) -> Result<()> {
            ...
            .map(|accessor| {
                if accessor.sparse().is_some() {
                        ...
                        buffer_metadatas.get(view.buffer().index()).unwrap()
                    {
                        ...
                        buffer_metadata.element_format = asset::Format::new(
                            asset::ElementFormat::from(accessor.data_type()),
                            accessor.dimensions().multiplicity(),
                        );
                        Ok(buffer_metadata)
                    } else {
                        Err(anyhow::anyhow!(
                            "No metadata found at {}",
                            view.buffer().index()
                        ))
                    }
                } else {
