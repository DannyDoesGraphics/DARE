...
pub fn update_frame_buffer(
    ...
    surface_added_query: becs::Query<(becs::Entity, &dare::engine::components::Surface, &dare::physics::components::Transform, &dare::render::components::BoundingBox), becs::Added<dare::engine::components::Surface>>,
    surface_changed_query: becs::Query<(becs::Entity, &dare::engine::components::Surface, &dare::physics::components::Transform, &dare::render::components::BoundingBox), becs::Changed<dare::engine::components::Surface>>,
    ...
    rt.clone().runtime.block_on(async {
        ...
        let mut deltas: Vec<dare::render::util::PersistentDelta<dare::render::c::CSurface>> = Vec::new();
        ...
        for (entity, surface, transform, bounding_box) in surface_changed_query {
            deltas.push(dare::render::util::PersistentDelta::Updated(
                entity.to_bits(), dare::render::c::CSurface::from_surface_zero(
                    &mut buffers,
                    surface,
                    transform,
                    bounding_box,
        ...
        for entity in surface_removed.read() {
            deltas.push(
                dare::render::util::PersistentDelta::Removed(entity.to_bits())
            )
        }
        // Submit deltas
        for frame in window_context.surface_context.as_mut().unwrap().frames.iter_mut() {
        ...
        if let Some(surface_context) = window_context.surface_context.as_mut() {
            let futures = surface_context.frames.iter_mut().filter_map(|frame| {
                ...
                Some(frame.surface_buffer_2.flush_queue(&transfer_context.immediate_submit))
            }).collect::<Vec<_>>();
