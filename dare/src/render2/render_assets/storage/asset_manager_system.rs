        ...
        for delta in buffer_storage.asset_server.get_deltas() {
            match delta {
                ...
                AssetServerDelta::HandleLoading(untyped_handle) => {
                    ...
                    if let Some(handle) = untyped_handle
                        .clone()
                        .into_typed_handle::<dare::asset2::assets::Buffer>()
                    {
                        match buffer_storage.insert(handle.clone()) {
                            ...
                            Ok(_) => {
                                ...
                                {
                                    if let Some(buffer_metadata) =
                                        buffer_storage.asset_server.get_metadata(&handle)
                                    {
                                        buffer_storage.load(
                                            &asset_storage_handle,
                                            dare::render::components::BufferPrepareInfo {
                        ...
                        untyped_handle.into_typed_handle::<dare::asset2::assets::Image>()
                    {
                        match image_storage.insert(handle.clone()) {
                            Ok(_) => {
                                ...
                                if let Some(asset_storage_handle) =
                                    image_storage.get_storage_handle(&handle)
                                {
                                    if let Some(image_metadata) =
                                        image_storage.asset_server.get_metadata(&handle)
                                    {
                                        image_storage.load(
                                            &asset_storage_handle,
                                            (
                                    ...
                                    tracing::warn!(
                                        "Expected `Some`, got `None` for handle {:?}",
                                        handle
                                    );
                                }
                            }
                ...
                AssetServerDelta::HandleUnloading(untyped_handle) => {
                    ...
                    if let Some(handle) =
                        untyped_handle.into_typed_handle::<dare::asset2::assets::Buffer>()
                    {
                        if let Some(render_asset_handle) =
                            buffer_storage.get_storage_handle(&handle)
                        {
                            buffer_storage
                                .handle_references
                                .get_mut(&*render_asset_handle)
                                .map(|mut v| {
                                    *v -= 1;
                                });
                        }
