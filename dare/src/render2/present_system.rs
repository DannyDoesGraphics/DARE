...
pub fn present_system_begin(mut frame_count: becs::Res<'_, super::frame_number::FrameCount>, render_context: becs::Res<'_, super::render_context::RenderContext>) {
    ...
    tokio::runtime::Handle::current().block_on(async move {
        ...
        let mut frame_guard = surface_context.frames[frame_number % surface_context.frames_in_flight].lock().await;
        ...
        *surface_context.swapchain_image_index.write().await = swapchain_image_index;
        ...
        frame.command_buffer.begin(vk::CommandBufferUsageFlags::ONE_TIME_SUBMIT).unwrap();
            ...
            surface_context.allocator.device()
                           ...
                           .cmd_clear_color_image(
                               ...
                               *frame.draw_image.as_raw(),
                               vk::ImageLayout::GENERAL,
                               &vk::ClearColorValue {
                                  float32: [((((frame_number as f64) / 200.0).cos() as f32) + 1.0)/2.0, 0.0, 0.0, 0.0],
                              },
                               &[
...
pub fn present_system_end(mut frame_count: becs::Res<'_, super::frame_number::FrameCount>, render_context: becs::Res<'_, super::render_context::RenderContext>) {
        ...
        }
        {
            ...
            };
            {
                frame.command_buffer
                    .submit(
                        *window_context.present_queue.acquire_queue_lock().await.unwrap(),
                        &[submit_info],
                ...
                unsafe {
                    match surface_context
                        .swapchain
                        .get_ext()
                        .queue_present(*window_context.present_queue.acquire_queue_lock().await.unwrap(), &present_info)
                    {
                        Ok(_) => {}
